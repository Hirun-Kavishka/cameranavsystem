...
# Load the reference image (loc00.jpg)
...
pixels_to_meters = 0.1  # Adjust this value according to your conversion factor

# Initialize an array to store the relative positions
relative_positions = []

# Create a BRISK object
brisk = cv2.BRISK_create()

# Detect keypoints and compute descriptors for the reference image
keypoints_ref, descriptors_ref = brisk.detectAndCompute(reference_gray, None)
...
for i in range(1, 5):
    ...
    keypoints_current, descriptors_current = brisk.detectAndCompute(current_gray, None)
